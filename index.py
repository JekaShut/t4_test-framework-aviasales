...
class Game:
   ...
   #    pass
    ...
    def selectBoard(self): # Выбор доски для игры
    ...
    def startGame(self, stop = 0, num=0, board = [board1, board2, board3]): # старт игры, вызов основных функций , проверка на завершение игры
        num = num
        while stop == 0:
            ...
            Game().actualTurn(board = board, num=num, actualPlayer = "X")  #self.actualPlayer,
            Game().checkWin(num = num) 
            Game().check_end(board = board, num = num) 
        else:
    ...
    def __boards(self, key = None, actualPlayer = "X", board = [board1, board2, board3], num = 0): # Проверка не занято ли место под крестик либо нолик
        ...
        if key == None:
            ...
            if actualPlayer == "X":
                ...
                if board[num][key] == "0" or board[num][key] == "X":
                ...
                if board[num][key] == "X" or board[num][key] == "0":
                    Game().randomSet()
                if board[num][key] == "-":
                    board[num][key] = actualPlayer
    ...
    def actualTurn(self, actualPlayer, board = [board1, board2, board3], num = 0):  # Мой ход
    ...
    def randomSet(self, num=0): # "Исскуственный интелект"
    ...
    def check_end(self, board = [board1, board2, board3], num = 0):   # проверка на то, закончились ли свободные клетки
    ...
    def display_board(self,num = 1):  # отрисовка доски
    ...
    def checkWin(self, num = 0): # проверка победы
        if num == 0: #3x3
